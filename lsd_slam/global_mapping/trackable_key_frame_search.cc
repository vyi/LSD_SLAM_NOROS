...
TrackableKeyFrameSearch::findEuclideanOverlapFrames(Frame* frame,
        float distanceTH, float angleTH, bool checkBothScales)
{
    ...
    for(unsigned int i=0; i<graph->keyframesAll.size(); i++)
    {
        ...
        Eigen::Vector3d otherViewingDir =
            graph->keyframesAll[i]->getScaledCamToWorld().rotationMatrix().rightCols<1>();
        ...
        potentialReferenceFrames.back().refToFrame = se3FromSim3(
                    graph->keyframesAll[i]->getScaledCamToWorld().inverse() *
                    frame->getScaledCamToWorld()).inverse();
...
{
    std::vector<TrackableKFStruct> potentialReferenceFrames =
        findEuclideanOverlapFrames(frame, maxScore / (KFDistWeight*KFDistWeight),
                                   0.75);
    ...
    for(unsigned int i=0; i<potentialReferenceFrames.size(); i++)
    {
        ...
        float score = getRefFrameScore(potentialReferenceFrames[i].dist,
                                       tracker->pointUsage);

        if(score < maxScore)
        {
            SE3 RefToFrame_tracked = tracker->trackFrameOnPermaref(
                                         potentialReferenceFrames[i].ref, frame,
                                         potentialReferenceFrames[i].refToFrame);
            ...
            float poseDiscrepancy = (potentialReferenceFrames[i].refToFrame *
                                     RefToFrame_tracked.inverse()).log().norm();
    ...
    bool closenessTH)
{
    ...
    std::vector<TrackableKFStruct> potentialReferenceFrames =
        findEuclideanOverlapFrames(keyframe,
                                   closenessTH * 15 / (KFDistWeight*KFDistWeight), 1.0 - 0.25 * closenessTH,
                                   true);
    for(unsigned int i=0; i<potentialReferenceFrames.size(); i++)
    ...
    if (enablePrintDebugInfo && printConstraintSearchInfo)
        printf("Early LoopClosure-Candidates for %d: %d euclidean, %d appearance-based, %d total\n",
               (int)keyframe->id(), (int)potentialReferenceFrames.size(), appearanceBased,
               (int)results.size());
