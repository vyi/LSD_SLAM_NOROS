...
UndistorterPTAM::UndistorterPTAM(const char* configFileName)
{
    ...
    if(valid)
    {
        ...
        else if(outputCalibration[0] == -1)	// "crop"
        {
            ...
            ofy = fy * ((top_radius + bottom_radius) / (trans_top_radius +
                        trans_bottom_radius)) * ((float)out_height / (float)in_height);
            ocy = (trans_top_radius/top_radius) * ofy*cy/fy;

            ofx = fx * ((left_radius + right_radius) / (trans_left_radius +
                        trans_right_radius)) * ((float)out_width / (float)in_width);
        ...
        else if(outputCalibration[0] == -2)	 // "full"
        {
            ...
            float trans_hor = std::max(trans_br_radius,
                                       trans_tr_radius) + std::max(trans_bl_radius,trans_tl_radius);
            float trans_vert = std::max(trans_tr_radius,
                                        trans_tl_radius) + std::max(trans_bl_radius,trans_br_radius);

            ofy = fy * ((vert) / (trans_vert)) * ((float)out_height / (float)in_height);
            ocy = std::max(trans_tl_radius/tl_radius,
                           trans_tr_radius/tr_radius) * ofy*cy/fy;

            ofx = fx * ((hor) / (trans_hor)) * ((float)out_width / (float)in_width);
            ocx = std::max(trans_bl_radius/bl_radius,
                           trans_tl_radius/tl_radius) * ofx*cx/fx;
...
void UndistorterPTAM::undistort(const cv::Mat& image,
...
UndistorterOpenCV::UndistorterOpenCV(const char* configFileName)
{
    ...
    K_ = cv::getOptimalNewCameraMatrix(camera_matrix, distortion_coefficients,
                                       cv::Size(img_width, img_height), outputCalibration, new_img_size, nullptr,
                                       false);
    cv::initUndistortRectifyMap(camera_matrix, distortion_coefficients, cv::Mat(),
                                K_, new_img_size, CV_16SC2, map1, map2);
...
void UndistorterOpenCV::undistort(const cv::Mat& image,
                                  cv::OutputArray result) const
{
