...
inline void NormalEquationsLeastSquares::update(const Vector6& J,
        const float& res, const float& weight)
{
...
void NormalEquationsLeastSquares::combine(const NormalEquationsLeastSquares&
        other)
{
    A_opt += other.A_opt;
    b += other.b;
...
inline void OptimizedSelfAdjointMatrix6x6f::rankUpdate(const
        Eigen::Matrix<float, 6, 1>& u, const float alpha)
{
    ...
    _mm_store_ps(data + 0, _mm_add_ps(_mm_load_ps(data + 0), _mm_mul_ps(v1122,
                                      v1212)));
    _mm_store_ps(data + 4, _mm_add_ps(_mm_load_ps(data + 4), _mm_mul_ps(v1122,
                                      v3434)));
    _mm_store_ps(data + 8, _mm_add_ps(_mm_load_ps(data + 8), _mm_mul_ps(v1122,
                                      v5656)));

    __m128 v3344 = _mm_mul_ps(s, _mm_unpacklo_ps(v3434, v3434));

    _mm_store_ps(data + 12, _mm_add_ps(_mm_load_ps(data + 12), _mm_mul_ps(v3344,
                                       v3434)));
    _mm_store_ps(data + 16, _mm_add_ps(_mm_load_ps(data + 16), _mm_mul_ps(v3344,
                                       v5656)));

    __m128 v5566 = _mm_mul_ps(s, _mm_unpacklo_ps(v5656, v5656));

    _mm_store_ps(data + 20, _mm_add_ps(_mm_load_ps(data + 20), _mm_mul_ps(v5566,
                                       v5656)));

#endif
}

inline void OptimizedSelfAdjointMatrix6x6f::operator +=(const
        OptimizedSelfAdjointMatrix6x6f& other)
{
#if defined(ENABLE_SSE)
    _mm_store_ps(data +  0, _mm_add_ps(_mm_load_ps(data +  0),
                                       _mm_load_ps(other.data +  0)));
    _mm_store_ps(data +  4, _mm_add_ps(_mm_load_ps(data +  4),
                                       _mm_load_ps(other.data +  4)));
    _mm_store_ps(data +  8, _mm_add_ps(_mm_load_ps(data +  8),
                                       _mm_load_ps(other.data +  8)));
    _mm_store_ps(data + 12, _mm_add_ps(_mm_load_ps(data + 12),
                                       _mm_load_ps(other.data + 12)));
    _mm_store_ps(data + 16, _mm_add_ps(_mm_load_ps(data + 16),
                                       _mm_load_ps(other.data + 16)));
    _mm_store_ps(data + 20, _mm_add_ps(_mm_load_ps(data + 20),
                                       _mm_load_ps(other.data + 20)));
...
void OptimizedSelfAdjointMatrix6x6f::toEigen(Eigen::Matrix<float, 6, 6>& m)
const
{
...
inline void NormalEquationsLeastSquares4::update(const Vector4& J,
        const float& res, const float& weight)
{
...
void NormalEquationsLeastSquares4::combine(const NormalEquationsLeastSquares4&
        other)
{
    A += other.A;
    b += other.b;
...
void NormalEquationsLeastSquares7::initializeFrom(const
        NormalEquationsLeastSquares& ls6, const NormalEquationsLeastSquares4& ls4)
{
    // set zero
    A.setZero();
    b.setZero();
...
void NormalEquationsLeastSquares7::combine(const NormalEquationsLeastSquares7&
        other)
{
    A += other.A;
    b += other.b;
