    ...
    int startLevel, int finalLevel)
{
    ...
    for(int lvl=startLevel; lvl >= finalLevel; lvl--)
    {
        ...
        if(buf_warped_size < 0.5 * MIN_GOODPERALL_PIXEL_ABSMIN * (width>>lvl)*
                (height>>lvl) || buf_warped_size < 10)
        {
            diverged = true;
            return Sim3();
        }

        Sim3ResidualStruct lastErr = callOptimized(calcSim3WeightsAndResidual,
                                     (referenceToFrame));
        if(plotSim3TrackingIterationInfo) callOptimized(calcSim3Buffers,(reference,
                    frame, referenceToFrame, lvl, true));
        ...
        for(int iteration=0; iteration < settings.maxItsPerLvl[lvl]; iteration++)
        {
            ...
            while(true)
            {
                ...
                Sim3 new_referenceToFrame =Sim3::exp(inc.cast<sophusType>()) *
                                           referenceToFrame;
                ...
                Sim3ResidualStruct error = callOptimized(calcSim3WeightsAndResidual,
                                           (new_referenceToFrame));
                if(plotSim3TrackingIterationInfo) callOptimized(calcSim3Buffers,(reference,
                            frame, new_referenceToFrame, lvl, true));
                numCalcResidualCalls[lvl]++;
    ...
    if (!warp_update_up_to_date)
    {
        ...
        callOptimized(calcSim3Buffers,(reference, frame, referenceToFrame,
                                       finalLevel));
    ...
    int level, bool plotWeights)
{
    ...
    Eigen::Matrix3f rollMat = shortestBackRotation.toRotationMatrix() *
                              rotMatUnscaled;
    ...
    const Eigen::Vector3f* refPoint_max = reference->posData[level] +
                                          reference->numData[level];
    ...
    for(; refPoint<refPoint_max; refPoint++, refGrad++, refColVar++)
    {
        ...
        *(buf_warped_z+idx) = Wxp(2);
        ...
        Eigen::Vector3f resInterp = getInterpolatedElement43(
                                        frame_intensityAndGradients, u_new, v_new, w);
        ...
        *(buf_d+idx) = 1.0f / (*refPoint)[2];
        ...
        if(plotSim3TrackingIterationInfo)
        {
            ...
            setPixelInCvMat(&debugImageOldImageWarped,getGrayCvPixel((float)resInterp[2]),
                            x,y,(width/w));
            setPixelInCvMat(&debugImageResiduals,getGrayCvPixel(residual_p+128),x,y,
                            (width/w));

            if(*(buf_warped_idepthVar+idx) >= 0)
            {
                setPixelInCvMat(&debugImageDepthResiduals,
                                getGrayCvPixel(128 + 800 * *(buf_residual_d+idx)),x,y,(width/w));

                if(plotWeights)
                {
                    setPixelInCvMat(&debugImageWeightD,
                                    getGrayCvPixel(255 * (1/60.0f) * sqrtf(*(buf_weight_VarD+idx))),x,y,(width/w));
                    setPixelInCvMat(&debugImageWeightedResD,
                                    getGrayCvPixel(128 + (128/5.0f) * sqrtf(*(buf_weight_VarD+idx)) * *
                                                   (buf_residual_d+idx)),x,y,(width/w));
                }
            }
            ...
            if(plotWeights)
            {
                setPixelInCvMat(&debugImageWeightP,
                                getGrayCvPixel(255 * 4 * sqrtf(*(buf_weight_VarP+idx))),x,y,(width/w));
                setPixelInCvMat(&debugImageHuberWeight,
                                getGrayCvPixel(255 * *(buf_weight_Huber+idx)),x,y,(width/w));
                setPixelInCvMat(&debugImageWeightedResP,
                                getGrayCvPixel(128 + (128/5.0f) * sqrtf(*(buf_weight_VarP+idx)) * *
                                               (buf_warped_residual+idx)),x,y,(width/w));
            }
        }
    ...
    const Sim3& referenceToFrame)
{
    ...
    const __m128 depthVarFacs = _mm_set1_ps((float)
                                            settings.var_weight);// float depthVarFac = var_weight;	// the depth var is over-confident. this is a constant multiplier to remedy that.... HACK
    ...
    for(int i=0; i<buf_warped_size-3; i+=4)
    {
        ...
        __m128 pz2ds = _mm_rcp_ps(_mm_mul_ps(_mm_mul_ps(pzs, pzs),
                                             _mm_load_ps(buf_d+i)));  // 1 / (z' * z' * d)
        ...
        if(i+3 < buf_warped_size)
        {
            ...
            sumResD = _mm_add_ps(sumResD,
                                 _mm_and_ps(depthValid, _mm_mul_ps(whs, _mm_mul_ps(weighted_rds,
                                            weighted_rds))));
    ...
    sumRes.sumResP = SSEE(sumResP,0) + SSEE(sumResP,1) + SSEE(sumResP,
                     2) + SSEE(sumResP,3);
    sumRes.numTermsP = (buf_warped_size >> 2) << 2;

    sumRes.sumResD = SSEE(sumResD,0) + SSEE(sumResD,1) + SSEE(sumResD,
                     2) + SSEE(sumResD,3);
    sumRes.numTermsD = SSEE(numTermsD,0) + SSEE(numTermsD,1) + SSEE(numTermsD,
                       2) + SSEE(numTermsD,3);

    sumRes.mean = (sumRes.sumResD + sumRes.sumResP) / (sumRes.numTermsD +
                  sumRes.numTermsP);
    ...
    const Sim3& referenceToFrame)
{
    ...
    float sum_rd=0, sum_rp=0, sum_wrd=0, sum_wrp=0, sum_wp=0, sum_wd=0,
          sum_num_d=0, sum_num_p=0;
    ...
    for(int i=0; i<buf_warped_size; i++)
    {
        ...
        float weighted_abs_res = sv > 0 ? weighted_rd+weighted_rp : weighted_rp;
        float wh = fabs(weighted_abs_res < settings.huber_d ? 1 : settings.huber_d /
                        weighted_abs_res);
...
void Sim3Tracker::calcResidualAndBuffers_debugFinish(int w)
{
    if(plotTrackingIterationInfo)
    {
        ...
        Util::displayImage( "Intensities of second_frame at pointcloud in first_frame",
